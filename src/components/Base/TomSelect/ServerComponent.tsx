...
async function fetchOptions({
  ...
  const response = url.includes('def14a') ? await axiosInstance.get(`${url}`) :await axiosInstance.get(`${url}?all=true`);
...
function TomSelect<T extends string | string[]>({
  ...
  const [fetchedOption, setFetchedOption] = useState<FetchedOptionType[]>([]);
